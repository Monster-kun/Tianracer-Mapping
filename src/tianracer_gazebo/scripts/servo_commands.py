...
total_speed = 0.0
speed_count = 0
world = os.getenv("TIANRACER_WORLD", "singapore")
...
_waypoints = utils.get_waypoints(filename)

# print(f"the waypoints are: {len(_waypoints)} waypoints")

def print_average_speed():
    if speed_count > 0:
        average_speed = total_speed / speed_count
        rospy.loginfo("Average speed before shutdown: {:.3f} m/s".format(average_speed))
    else:
        rospy.loginfo("No speed data recorded.")

def get_goal_point(data):
    global _waypoints, _counter
    _counter += 1
    if _counter == len(_waypoints):
        _counter = 0

def set_throttle_steer(data):
    global emergency_brake_active, total_speed, speed_count, max_speed
    # print(f"set throttle steer +=========================")
    ...
    if not emergency_brake_active:
        ...
        if (_counter == 5  ) and data.drive.speed > max_speed:
            data.drive.speed = max_speed
        if (_counter == 9) and data.drive.speed > 3.5:
            data.drive.speed = 3.5
        if (_counter == 7) and data.drive.speed > 4.5:
            data.drive.speed = 4.5
        # if (_counter == 1) and data.drive.speed > 5:
        #     data.drive.speed = 5
        if (_counter == 11 or _counter == 6 ) and data.drive.speed > 4.5:
            data.drive.speed = 4.5
        if ( _counter == 8) and data.drive.speed > 3:
            data.drive.speed = 3
        # if (_counter == 2 or _counter == 4 or _counter == 3 or _counter == 10) and data.drive.speed > 5.4:
        #     data.drive.speed = 6.5
        if (_counter == 2) and data.drive.speed > 5.4:
            data.drive.speed = 5.7
        throttle = data.drive.speed*31.25
        steering_angle = data.drive.steering_angle

        print(f"speed is: {data.drive.speed}, angle is: {steering_angle}")
...
def servo_commands():
    ...
    rospy.Subscriber("/tianracer/move_base/goal", MoveBaseActionGoal, get_goal_point)
    

    # Advertise emergency brake service
    rospy.Service('emergency_brake', Empty, emergency_brake)
    rospy.on_shutdown(print_average_speed)
