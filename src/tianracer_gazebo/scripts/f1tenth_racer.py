...
class RaceStateMachine(object):
    def __init__(self, filename, repeat=True):
        ...
        if not self._ac_move_base.wait_for_server(rospy.Duration(5.0)):
        ...
        self._min_tolerance_length = 100

        # pub first goal
        # rospy.sleep(0.3)     # wait for move_base to be ready, it is necessary and very important
        ...
        # rospy.loginfo("Waiting for global plan to appear...")
        # t0 = time.time()
        # while not self._len_path_list and time.time() - t0 < 5.0:
        #     rospy.sleep(0.05)
        # rospy.loginfo("Global plan received. Proceeding.")
        ...
        rospy.loginfo("the time of initialization: %f", time.time() - start_time)
    ...
    def _rest_of_path_callback(self, data):
        ...
        if not self._len_path_list:
            ...
            # self._tolerance_length = max(self._len_path_list) / 3    # controls how many transit points are left in the currently planned route and sends the next destination point
            self._tolerance_length = 220
            # limit of value
            ...
            if (self._counter == 7 ):
                self._tolerance_length = 90
  
            if (self._counter == 10 ):
                self._tolerance_length = 190
            if (self._counter == 11 ):
                self._tolerance_length = 100 # 原11
                # self._tolerance_length = 60
            if (self._counter == 9 ):
                self._tolerance_length = 170
            if (self._counter == 8):
                self._tolerance_length = 100
        else:
            pass
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown() and self._repeat:
            ...
            if self._early_pub:
                ...
                self.move_to_next()
                self._early_pub = False
                # rospy.loginfo("the time of spin: %f", time.time() - start_time)
...
if __name__ == '__main__':
    ...
    # rospy.loginfo("the time of main: %f", time.time() - start_time)
    m.spin()
